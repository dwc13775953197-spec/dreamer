...
3. 调用 `session_search(query="*", sort="newest", limit=10)` 检索上次 walk 之后的会话
4. 筛选包含用户消息（role=user）的会话
5. 提取关键内容：用户说了什么？提出了什么新问题？表达了什么新观点？
6. 将提取内容作为"外部认知信号"纳入本次散步的感知输入
