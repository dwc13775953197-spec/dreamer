...
**成熟度计算**：对 subconscious.json 中每条 active 条目计算：

```
成熟度 = 0.30 × (连接数 / 5) + 0.25 × (被引用次数 / 5) + 0.25 × (存在天数 / 30) + 0.20 × (DORMANT 周期数 / 10)
```

- 连接数 = `connections` 数组长度
- 被引用次数 = 其他条目 `connections` 中引用该 ID 的次数
- 存在天数 = (当前时间 - created) 的天数
- DORMANT 周期数 = `decayed_at` 被更新的次数
- 所有比值 clamp 到 [0, 1]

**系统健康度判断**：先统计全部 active 条目的成熟度分布，确定当前系统处于什么阶段：

| 阶段 | 条件 | 含义 |
|---|---|---|
| 生长期 | > 70% 条目成熟度 < 0.2 | 大量一次性 insight 未被消化 |
| 成长期 | 30-70% 条目成熟度 < 0.2 | 有足够材料开始建立连接 |
| 成熟期 | > 50% 条目成熟度 0.3-0.7 | 系统健康，正常交替即可 |
| 饱和期 | > 50% 条目成熟度 > 0.7 | 材料太熟，需要新输入 |

**选择规则（按系统阶段）**：

- **生长期**（> 70% 太生）→ **聚焦型探索**
  - 不产出新条目，而是深入处理当前最强的 1 条 entry
  - 花整次散步时间深挖它，产出 1 条深度 insight（引用原始条目 + 外部材料）
  - 目标：让这条 entry 的成熟度从 < 0.2 提升到 0.3+
  - 如果最强 entry 的 strength < 2.0 → 先跳过它，选 strength 最高的

- **成长期**（30-70% 太生）→ **标准巩固型**
  - 选成熟度 0.3-0.5 的条目建立连接
  - 优先选 intensity >= 4 的种子关联的材料
  - 如果所有关联材料成熟度 > 0.7 → 探索型引入新领域

- **成熟期**（正常状态）→ **标准动态选择**
  - 按动态反馈控制规则选择探索/巩固（见上方散步类型选择）
...
散步结束后，计算本次散步对引用图结构的影响：

```
1. 提取散步前引用图快照：对每个 active entry，记录其 connections 集合
2. 提取散步后引用图快照：更新后的 connections 集合
3. 计算对称差：变化量 = |{散步前} △ {散步后}| （新增 + 删除的边数）
4. 变化率 = 变化量 / max(总连接数, 1)，以百分比表示
```
...
**反馈规则**：
- 连续 2 次变化率 < 5% → 下次强制切换散步类型
- 连续 3 次变化率 > 15% → 系统处于高产出期，维持当前类型
- 连续 5 次变化率 < 5% → 强制异域散步（搜索不相关领域）

记录变化率到散步日志 frontmatter 的 `graph_change_rate` 字段。
